...
const fs = require("fs");
//引用formidable来解析请求，上传文件
const formidable = require("formidable");
//生成时间对象，来重命名上传的图片，避免图片的重复
...
module.exports = {
  ...
  uploadPhoto: function(req, dirName, callback) {
    ...
    form.uploadDir = path.join(__dirname, "../uploads", dirName);
    //如果文件夹不存在则创建
    ...
    form.parse(req, function(err, fields, files) {
      ...
      var newPath = path.join(__dirname, "../uploads", dirName, newName);
      //修改文件的名字
      fs.renameSync(oldPath, newPath);
      var finalPath = path
        .join("/", dirName, newName)
        .split("\\")
        .join("/"); //将路径中的'\'替换为'/'
